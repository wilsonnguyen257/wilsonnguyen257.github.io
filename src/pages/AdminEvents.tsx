...
const AdminEvents = () => {
  ...
  // Migration function: Convert description → content
  const migrateDescriptionToContent = async () => {
    if (!confirm('This will migrate all events with "description" to use "content" field. Continue?')) {
      return;
    }
    ...
    try {
      ...
      const eventsRaw = await getJson<any[]>('events');
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const migratedEvents = allEvents.map((event: any) => {
        // Already has content
        ...
        // Has description to migrate
        if (event.description) {
          migratedCount++;
          return {
            ...event,
            content: event.description,
            // Keep description for safety
          };
        }
      ...
      await saveJson('events', migratedEvents);
      
      // Reload events
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const refreshed: Event[] = (migratedEvents || []).map((d: any) => ({
        id: d.id,
      ...
      setMigrationResult(`✅ Migration complete! Migrated: ${migratedCount}, Skipped: ${skippedCount}`);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 py-8 px-4 sm:px-6 lg:px-8 transition-colors">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-amber-800 dark:text-amber-200 mb-1">
                {language === 'vi' ? 'Chuyển đổi dữ liệu' : 'Data Migration'}
              </h3>
      ...
      {migrating && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 px-6 py-4 rounded-r-lg mb-6 shadow-md">
          <div className="flex items-center gap-3">
            ...
            <span className="text-blue-800 dark:text-blue-200">
              {language === 'vi' ? 'Đang chuyển đổi dữ liệu...' : 'Migrating data...'}
            </span>
          </div>
        </div>
      )}
      ...
      {migrationResult && (
        <div className={`border-l-4 px-6 py-4 rounded-r-lg mb-6 shadow-md ${
          migrationResult.startsWith('✅')
            ? 'bg-green-50 dark:bg-green-900/20 border-green-500'
            : 'bg-red-50 dark:bg-red-900/20 border-red-500'
        }`}>
          <div className="flex items-start gap-3">
            <span className={migrationResult.startsWith('✅') ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'}>
              {migrationResult}
            </span>
            <button
