    ...
    <div className="container-xl py-8">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-8 border border-slate-100">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
          ...
          <h2 className="text-xl font-semibold text-slate-900">{t('admin.gallery.upload_new')}</h2>
        </div>

        <p className="text-sm text-slate-600 mb-6 flex items-start gap-2 bg-brand-50 p-3 rounded-lg border border-brand-100">
          ...
          <div className="flex-1">
            <label
              ...
              className={`flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-xl cursor-pointer transition-all ${
                file ? 'border-brand-500 bg-brand-50' : 'border-slate-300 bg-slate-50 hover:bg-slate-100'
              }`}
            >
      ...
      {paginatedImages.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-12 text-center border border-slate-100">
          ...
          <p className="text-slate-500 font-medium text-lg">{searchTerm ? 'No matching images found' : t('admin.gallery.no_images')}</p>
        </div>
      ) : (
        <>
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {paginatedImages.map((img) => (
              <div key={img.id} className="relative group bg-white rounded-xl shadow-sm overflow-hidden border border-slate-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  ...
                  {editingId === img.id ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={cancelEdit}
                          className="flex-1 inline-flex items-center justify-center gap-1 px-2 py-1.5 bg-slate-100 text-slate-700 text-xs font-semibold rounded-lg hover:bg-slate-200 transition-colors"
                        >
          ...
          {totalPages > 1 && (
            <div className="flex justify-center mt-8 gap-2">
                <button
                    ...
                    className="px-4 py-2 border rounded-lg bg-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 transition-colors font-medium text-slate-700"
                >
                    {t('admin.pagination.prev') || 'Previous'}
                </button>
                <span className="px-4 py-2 bg-white border rounded-lg text-slate-700 font-medium">
