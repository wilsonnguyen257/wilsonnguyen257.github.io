...
export default function Gallery() {
  ...
  type GalleryImage = { id: string; url: string; name: string; created: number };
  ...
  setImages(querySnapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as Omit<GalleryImage, 'id'>) })));
    };
    fetchImages();
  }, []);
  ...
  useEffect(() => {
    if (images.length || images.length === 0) setLoading(false);
  }, [images]);
  ...
  <h1 className="h1 text-center">{t('gallery.title')}</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-20">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-brand-300 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M9 10l3 3m0 0l3-3m-3 3V4" /></svg>
